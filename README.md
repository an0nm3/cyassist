...
# Cyassist v3.0 🇮🇳 — Cybersecurity News Reader

Daily cybersecurity news feed archive — auto-collected from 35+ RSS and Telegram sources. Browse, search, and filter security news from the terminal. Built for the Indian cybersecurity and security community.
...
```bash
git clone https://github.com/an0nm3/cyassist.git
cd cyassist
...
Fetch the latest articles into the local archive:

```bash
# Indian sources (CERT-In, ET CISO, RBI advisories, etc.)
python3 cyassist.py --news-india

# Global web sources (THN, BleepingComputer, Reddit, X)
python3 cyassist.py --news-web

# Both (daily run)
python3 cyassist.py --daily
```
